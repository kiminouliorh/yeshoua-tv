  ...
  return (
    <main className="min-h-screen bg-[#050505] text-white font-sans selection:bg-red-600/30">
      ...
      <div className="pt-24 pb-32 px-4 max-w-5xl mx-auto">
        ...
        {/* --- PAGE DIRECT (Bouton Partage Remis à sa place) --- */}
        {currentTab === 'direct' && (
          <section className="animate-in fade-in duration-500">
            <div className="relative mb-12">
              ...
              {/* TON BOUTON PARTAGE COLLÉ AU LECTEUR */}
              <button 
            ...
            <p className="text-zinc-600 text-[9px] font-bold uppercase tracking-[0.3em] mt-2 italic">Direct d'Abidjan • Ministère de la Foi</p>
          </section>
        )}
        ...
        {currentTab === 'programme' && (
          <section className="animate-in slide-in-from-right duration-500">
            <h2 className="text-2xl font-black uppercase italic mb-8 underline decoration-red-600 underline-offset-8">Planning de la semaine</h2>
            ...
            <div className="space-y-4">
              {(planning[activeDay] || [{h:"--", t:"Aucun programme"}]).map((p, i) => (
                <div key={i} className="flex items-center gap-6 p-5 bg-zinc-900/40 rounded-2xl border border-white/5 backdrop-blur-sm transition-all hover:bg-zinc-900">
            ...
            <div className="grid grid-cols-2 gap-4">
              {[1, 2, 3, 4].map(i => (
                <div key={i} className="group cursor-pointer">
                  ...
                  <h4 className="text-[10px] font-black uppercase tracking-tight text-zinc-500 group-hover:text-white">Culte #{i}</h4>
                </div>
              ))}
            </div>
          </section>
      ...
      {/* MODALE DE PARTAGE PRO (WhatsApp, FB, X, Copier) */}
      {showShare && (
        <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-4 animate-in fade-in duration-300">
          ...
          <div className="relative bg-[#111] w-full max-w-sm rounded-[2.5rem] p-8 border border-white/10 shadow-2xl">
            <h3 className="text-xl font-black uppercase text-center mb-8">Diffuser la parole</h3>
            <div className="grid grid-cols-3 gap-4 mb-10 text-center">
              {shareOptions.map(opt => (
                <a key={opt.name} href={opt.url} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center gap-2">
                   ...
                   <span className="text-[8px] font-bold text-zinc-500 uppercase">{opt.name}</span>
                </a>
              ))}
            </div>
            <button onClick={() => { navigator.clipboard.writeText(SITE_URL); alert("Lien copié !"); setShowShare(false); }} className="w-full bg-white text-black py-4 rounded-2xl font-black text-xs uppercase tracking-widest mb-2">Copier le lien</button>
            <button onClick={() => setShowShare(false)} className="w-full py-2 text-zinc-600 font-bold text-[9px] uppercase tracking-widest">Fermer</button>
          </div>
        </div>
      )}

      {/* NAVIGATION BASSE FIXE (3 BOUTONS) */}
      <nav className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[85%] max-w-sm z-50">
        <div className="bg-zinc-900/95 backdrop-blur-3xl border border-white/10 rounded-2xl p-1.5 flex justify-around shadow-2xl ring-1 ring-white/5">
          <button onClick={() => setCurrentTab('programme')} className={`flex flex-col items-center gap-0.5 px-4 py-1.5 transition-all ${currentTab === 'programme' ? 'text-white scale-110' : 'text-zinc-600'}`}>
            ...
            <span className="text-[8px] font-black uppercase tracking-tighter">Programme</span>
          </button>
          <button onClick={() => setCurrentTab('direct')} className={`flex flex-col items-center gap-0.5 px-4 py-1.5 transition-all ${currentTab === 'direct' ? 'text-red-500 scale-110' : 'text-zinc-600'}`}>
          ...
          <button onClick={() => setCurrentTab('replays')} className={`flex flex-col items-center gap-0.5 px-4 py-1.5 transition-all ${currentTab === 'replays' ? 'text-white scale-110' : 'text-zinc-600'}`}>
